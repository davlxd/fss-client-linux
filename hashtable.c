...
int rolling_to_file(const char *in_path, const char *out_path, int with_sha1)
{
  FILE *in, *out;
  size_t len;
  char buf[BLOCK_LEN];
  ...
  while(len) {
    if (with_sha1) {
      ...
      len = snprintf(record, RECORD_LEN+1, "%08"PRIX32"\t%s\n",
		     get_checksum1(buf, (int32_t)len), sha1);
    } else {
